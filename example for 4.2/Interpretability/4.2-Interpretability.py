...
# create dataset
df  = np.loadtxt("example-2.txt", skiprows=1)
...
if mode == "manual":
    # manual mode
    model.fix_symbolic(0, 0, 0, 'sin', a_range=(5, 6));
    model.fix_symbolic(0, 0, 1, 'sin', a_range=(4, 5));
    model.fix_symbolic(0, 0, 2, 'sin', a_range=(6, 7));
   # model.fix_symbolic(0, 0, 3, 'cos', a_range=(5, 6));
    #model.fix_symbolic(0, 0, 4, 'cos', a_range=(4, 5));
    #model.fix_symbolic(0, 0, 5, 'cos', a_range=(6, 7));
...
# 读取excel表格数据
data = pd.read_excel('Interpretability-2.xlsx',sheet_name='Sheet1',header=1)
